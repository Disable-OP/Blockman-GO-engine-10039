...
namespace LORD
{
	...
	{
		auto components = StringUtil::Split(path, "/\\");
	...
	String PathUtil::GetAbsolutePath(const String& path)
	{
		...
		{
			return NormalizePath(ConcatPath(GetCurrentDir(), path));
		}
	}
...
#else
				if (mkdir(tempDir.c_str(), S_IRWXU | S_IRWXG | S_IRWXO))
#endif
				{
						LordLogError("PathUtil::CreateDir error code = [%d] , dir = [%s]", errno, tempDir.c_str());
						return false;
				}
			}
		}
	...
	bool PathUtil::CopyFilePath(const String& src, const String& dest, bool bOverWrite)
	{
		...
		filesrc = fopen(src.c_str(), "rb");
		if (filesrc == 0)
		{
			return false;
		}
		filedst = fopen(dest.c_str(), "wb");
		if (filedst == 0)
		{
			fclose(filesrc);
			return false;
		...
		char buf[4096];
		size_t rdCount = fread(buf, sizeof(char), 4096, filesrc);
		while (rdCount != 0)
		{
			fwrite(buf, sizeof(char), rdCount, filedst);
			rdCount = fread(buf, sizeof(char), 4096, filesrc);
		}
		fclose(filesrc);
		fclose(filedst);
	...
	bool PathUtil::CopyDir(const String& srcDir, const String& destDir, bool bOverWrite)
	{
		...
		while((pDirInfo = readdir(pDir)) != NULL)
		{
			...
			else
			{
				...
				PathUtil::CopyFilePath(dirname, destFile, bOverWrite);
			}
		} // while
		closedir(pDir);
		return true;
