...
namespace LORD
{
	...
	public:
		...
		Vector3 getIntermediateWithXValue(const Vector3& v1, float ratio) const
		{
			...
			{
				Real var10 = (ratio - x) / delta.x;
				if (var10 >= 0.f && var10 <= 1.f)
					return *this + delta*var10;
			}
			return Vector3::INVALID;
		}

		Vector3 getIntermediateWithYValue(const Vector3& v1, float ratio) const
		{
			Vector3 delta = v1 - *this;
		...
		Vector3 getIntermediateWithZValue(const Vector3& v1, float ratio) const
		{
			Vector3 delta = v1 - *this;
