...
namespace LORD
{
	...
	{
		char tmpBuf[LORD_STREAM_TEMP_SIZE];
		String retString;
		size_t readCount;
		// Keep looping while not hitting delimiter
		while ((readCount = read(tmpBuf, LORD_STREAM_TEMP_SIZE-1)) != 0)
		{
			// Terminate string
			tmpBuf[readCount] = '\0';
	...
	size_t DataStream::readLine(char* buf, size_t maxCount, const String& delim)
	{
		...
		char tmpBuf[LORD_STREAM_TEMP_SIZE];
		size_t chunkSize = std::min<size_t>(maxCount, (size_t)LORD_STREAM_TEMP_SIZE-1);
		size_t totalCount = 0;
		size_t readCount; 
		while (chunkSize && (readCount = read(tmpBuf, chunkSize)) != 0)
		{
			...
			chunkSize = std::min<size_t>(maxCount-totalCount, (size_t)LORD_STREAM_TEMP_SIZE-1);
		}
	...
	//-----------------------------------------------------------------------
	String DataStream::readString(void)
	{
		ui16 strSize = 0;
		read(&strSize, sizeof(ui16));

		char* pBuf = (char*)LordMalloc(strSize + 1);
		read(pBuf, strSize);
		pBuf[strSize] = '\0';
		String str;
		str.insert(0, pBuf, strSize);
		LordFree(pBuf);
		return str;
	}
	//-----------------------------------------------------------------------
	String DataStream::getAsString(void)
	{
		...
		char* pBuf = (char*)LordMalloc(mSize+1);
	...
	//-----------------------------------------------------------------------
	void DataStream::writeString(const String& str)
	{
		ui16 strSize = ui16(str.length());
		write(&strSize, sizeof(ui16));
		...
		const String& delim)
	{
		if (delim.empty())
		{
			LordException("Error: No delimiter provided ------ FileStreamDataStream::readLine");
		}
		if (delim.size() > 1)
		{
			LordException("WARNING: FileStreamDataStream::readLine ------ using only first delimeter");
		}
		// Deal with both Unix & Windows LFs
		bool trimCR = false;
		if (delim.at(0) == '\n') 
		...
		else if (mpInStream->fail())
		{
			...
			{
				LordException("Error: Streaming error occurred ------- FileStreamDataStream::readLine");
			}
		}
		else 
		{
	...
	void FileStreamDataStream::close(void)
	{
		if (mpInStream)
		{
			...
			if (mFreeOnClose)
			{
				...
				if (mpFStreamRO)
					LordDeleteT(mpFStreamRO, basic_ifstream);
				if (mpFStream)
					LordDeleteT(mpFStream, basic_fstream);
				mpInStream = 0;
				mpFStreamRO = 0; 
				mpFStream = 0; 
			}
	...
	//-----------------------------------------------------------------------
	//-----------------------------------------------------------------------
	FileHandleDataStream::FileHandleDataStream(FILE* handle, ui16 accessMode)
	...
	//-----------------------------------------------------------------------
	FileHandleDataStream::FileHandleDataStream(const String& name, FILE* handle, ui16 accessMode)
