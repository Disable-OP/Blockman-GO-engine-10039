...
public:
        ...
        // Server-authoritative world generation, step 2 (population):
        // ChunkProviderGenerate only carves terrain + biome surface + caves
        // (step 1). Trees, flowers, ores, mushrooms, cacti, etc. come from
        // BiomeDecorator::decorate — which was never wired up before. This
        // method decorates chunk (x, z) ONCE (guarded by the chunk's
        // m_isTerrainPopulated flag, which is also persisted in Anvil NBT).
        // Vanilla semantics: decoration of (x,z) requires the +x, +z and
        // +x+z neighbour chunks to be GENERATED, because trees/structures
        // may spill over the chunk border. Called from the chunk-request
        // packet handler before the chunk is serialized to the client, so
        // the client always receives a fully-populated chunk.
        // Runs on the server tick thread (packet handlers are dispatched
        // from ServerNetwork::logicTick → recvTick), same as all other
        // world mutation — no extra locking needed.
        void populateChunk(int x, int z);
...
public:
        ...
        /** Counter for the periodic saveAllChunks flush in tick() (600 ticks ≈ 30 s). */
        int m_saveTickCounter = 0;
